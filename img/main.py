...
from PIL import Image as PILImage  # Import do manipulacji obrazami (PIL)
...
# Tworzymy konfigurację dla zdjęć z użyciem metody create_still_configuration
capture_config = picam2.create_still_configuration(
    main={"format": "BGR888"},  # Możesz ustawić format obrazu
    colour_space=ColorSpace.Srgb()  # Przestrzeń kolorów sRGB
    # Brak klucza "transform" oznacza brak transformacji
)

# Zastosowanie pełnej konfiguracji kamery
...
def capture():
    ...
    # Zrób zdjęcie i zapisz je
    file_path = f'/home/tf88lk/img/images/{timestamp}.jpg'
    picam2.capture_file(file_path)
    picam2.stop()  # Zatrzymanie kamery po wykonaniu zdjęcia

    # Sprawdź rozmiar zdjęcia
    file_size = os.path.getsize(file_path)  # W bajtach
    file_size_kb = file_size / 1024  # Przekształć do KB
    file_size_mb = file_size_kb / 1024  # Przekształć do MB

    # Obróć obrazek o 180 stopni (w pionie)
    image_pil = PILImage.open(file_path)
    image_pil = image_pil.rotate(180)  # Obrót o 180 stopni
    image_pil.save(file_path)  # Zapisz obrócony obrazek
    ...
    # Zaktualizuj komunikat na OLED
    show_photo_message = True
    time.sleep(3)  # Czas wyświetlania komunikatu
    show_photo_message = False
...
def oled_loop():
    while True:
        ...
        if show_photo_message:
            ...
            # Wyświetlanie standardowych danych systemowych
